...
本文件给 Codex (Codex.ai/code) 在本仓库工作时提供导航。子模块的细节走 `DESIGN.md`；本文只覆盖项目骨架 + 跨模块约定。
...
- **Agent 投研系统** (`agents/`)：Codex subagent 模式驱动因子迭代研究，Python 侧只保留执行层
- **交易模块** (`trading/`)：第一阶段只做策略信号推送 + 本地仓位跟踪，不直连券商

数据流：

```
研报/论文 → Codex (agent subagents / skills / MCP)
                    ↓                              ↑
...
OPENAI_API_KEY=...       # Codex/OpenAI API（TBD）
...
入口文档：[`backtest/AGENTS.md`](backtest/AGENTS.md) + [`backtest/PIPELINE.md`](backtest/PIPELINE.md)。
