...
交互式因子研究入口：用户给出一个自然语言因子猜测后，由 Codex 直接生成因子代码、运行单轮回测、分析结果、写入 trace，并在同方向上修复或调参，直到达标或停止。

**Phase 1 增强**：失败时启动 Result Critic subagent（Codex subagent）诊断 + 查 KB，替代硬编码 if/else repair policy。
...
- 失败诊断：先执行 **Pre-RC Strategy Sweep Fast Path**（强因子仅策略回测失败时直接 sweep）；不满足 fast path 时再启动 Result Critic subagent（Codex subagent），由其读取 result.json + trace.jsonl + KB，输出结构化诊断 JSON。父进程根据 RC 输出决定 repair / abandon / 换方向。
...
4. Generate or repair one factor implementation.
   ...
   - Use only schema columns returned by `codex_cli schema`.
...
5. Write the factor code + config:
   ...
   **Round 1 或 factor_change="formula"**（公式/代码结构变化）：
   - 因子代码**只写入** `alphas/exp/agent/<factor_id>/factor.py`
   - 策略配置写入 `alphas/exp/agent/<factor_id>/config.yaml`

   **factor_change="params"**（窗口/horizon/variant/公式参数变化）：
   - 必须更新 `alphas/exp/agent/<factor_id>/factor.py` 中的 `@register(parameters=...)`、`variant=...` 或公式常量
   - 同时按需要更新 `alphas/exp/agent/<factor_id>/config.yaml`
   - 因子值会改变，不能复用旧的 factor_eval/decile_backtest

   **strategy_only**（仅策略提取参数变化）：
...
6. Run pipeline（区分两类修复）：
   ...
   **Round 1 / factor_change="formula" / factor_change="params"**（因子值可能变化，先跑因子评估 step1~4）：
   ```bash
   ...
   **factor_change="params"**（窗口/horizon/variant 等因子值变化）：确认已更新 factor.py / registry metadata 后，从 step1 重新运行，因为因子值已改变。
...
10. **If `result.json.status != "pass"` 且不满足 Pre-RC fast path**：启动 Result Critic subagent 诊断。
   ...
   **条件注入：父进程组装 RC Prompt**
   ...
   d. **组装完整 prompt**：
      - Role (from `.codex/prompts/shared/role.md` RC persona)
      ...
      - Task + Output Format (from `.codex/prompts/result_critic.md`)
   ...
   **完整 Diagnosis JSON Schema** 见 `.codex/prompts/shared/output_formats.md`。
...
2. **Auto-append trace**（若 run 时未使用 `--auto-kb-update`）：
   ```bash
   conda activate AutoQuant && python -m agents.codex_cli trace-append \
...
Use aliases returned by `codex_cli schema`:
