...
class TestNeutralizer:
    """Test industry and market-cap neutralization."""

    def test_industry_group_rank(self):
        """Test industry neutralization by group rank."""
        from backtest.strategy.neutralize import Neutralizer

        factor = pd.Series({"A": 1.0, "B": 3.0, "C": 2.0, "D": 4.0})
        industry = pd.Series({"A": "Tech", "B": "Tech", "C": "Finance", "D": "Finance"})

        n = Neutralizer()
        result = n.industry_neutralize(factor, industry, method="group_rank")

        # Within Tech: A=1, B=3 -> ranks [0.5, 1.0] (pandas rank pct = rank/count)
        # Within Finance: C=2, D=4 -> ranks [0.5, 1.0]
        assert result["A"] == pytest.approx(0.5)
        assert result["B"] == pytest.approx(1.0)
        assert result["C"] == pytest.approx(0.5)
        assert result["D"] == pytest.approx(1.0)

    def test_industry_group_zscore(self):
        """Test industry neutralization by group z-score."""
        from backtest.strategy.neutralize import Neutralizer
        ...
        n = Neutralizer()
        result = n.industry_neutralize(factor, industry, method="group_zscore")

        # Tech: mean=2, std=sqrt(2) -> A=-1/sqrt(2), B=1/sqrt(2)
        assert result["A"] == pytest.approx(-1 / np.sqrt(2))
        assert result["B"] == pytest.approx(1 / np.sqrt(2))
        # Finance: mean=12, std=sqrt(8) -> C=-1/sqrt(2), D=1/sqrt(2)
        assert result["C"] == pytest.approx(-1 / np.sqrt(2))
        assert result["D"] == pytest.approx(1 / np.sqrt(2))

    def test_industry_single_member_group(self):
        """Test handling of groups with only one member."""
        from backtest.strategy.neutralize import Neutralizer

        factor = pd.Series({"A": 1.0, "B": 2.0, "C": 3.0})
        industry = pd.Series({"A": "Tech", "B": "Tech", "C": "Solo"})

        n = Neutralizer()
        result = n.industry_neutralize(factor, industry, method="group_zscore")

        # Solo group: std=0, should return 0.0
        assert result["C"] == pytest.approx(0.0)

    def test_market_cap_neutralize(self):
        """Test market-cap neutralization removes linear relationship."""
        from backtest.strategy.neutralize import Neutralizer

        # Create factor with market cap exposure plus noise
        factor = pd.Series({"A": 2.0, "B": 3.0, "C": 5.0, "D": 6.0, "E": 8.0})
        cap = pd.Series({"A": 1e8, "B": 1e9, "C": 1e10, "D": 1e11, "E": 1e12})

        n = Neutralizer()
        result = n.market_cap_neutralize(factor, cap)

        # Residual should have near-zero correlation with log(cap)
        log_cap = np.log(cap[result.index])
