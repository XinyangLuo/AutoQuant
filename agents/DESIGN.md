...
Subagent 类型（4 个）：
  ...
  └── RC — Result Critic          诊断 + 修复决策
        ↓
Python 执行层（agents/）
  ├── claude_cli.py  — schema / run / trace-append / kb-update
...
| 流水线执行 | `runner.py` | `claude_cli.py` | `AutoQuantFactorRunner.run(experiment)` |
| 反馈评估 | `evaluator.py` | `claude_cli.py` | `AutoQuantFactorEvaluator.evaluate(experiment)` → `QuantFeedback` |
| Trace 读写 | `trace.py` | `claude_cli.py` | `TraceManager.append(record)` / `read_all()` |
| KB 查询 | `kb_query.py` | 父进程 / RC prompt | CLI → JSON |
| KB 更新 | `kb_update.py` | `claude_cli.py` | `KbUpdater.update_on_pass/fail(experiment)` |
...
- **自动化**：`claude_cli.py run` 在 `--run-dir` 模式下自动追加 trace，替代完全手动的文件操作
- **DAG 支持**：`parent_round_id` + `branch_id` 支持线性迭代和分支探索
- **纯函数**：`TraceRecord.from_result_json()` 只接收 dict，不读取文件，便于单元测试

#### 3.1.2 TraceRecord 字段

对齐 `.claude/prompts/shared/output_formats.md` 中的 Trace JSONL Schema。关键字段：
...
- **CLI 子命令**：`python -m agents.claude_cli kb-update --result <path> --status pass|fail [--rc-output <path>]`
- **自动触发**：`cmd_run` 新增 `--auto-kb-update` 标志，run 结束后自动调用
- 默认不启用（向后兼容），`/factor-iterate` 迭代模式建议启用
...
Prompt 模板固定（存放在 `.claude/prompts/`），但父进程按场景动态选择注入哪些 section。
...
所有 subagent 输出结构化 JSON，schema 定义见 `.claude/prompts/shared/output_formats.md`：
...
[FC] 生成 factor.py + config.yaml
    ↓
python -m agents.claude_cli run <factor_id> --run-dir <dir>
...
```
PDF → MCP pdfplumber 提取 → 因子穷举 JSON → [HO] 评审排序
    ↓
用户审阅 → hypothesis.md
    ↓
