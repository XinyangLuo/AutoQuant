...
Pipeline 每个 step 只执行一次，不做自动重试。当 step6 或 step7 不通过时：

1. **调用者（Agent 或人工）**分析失败原因，决定新参数
2. 通过 `config.yaml` 更新默认值（持久）或 CLI `--top-k` / `--decay` / `--rebalance` 覆盖（一次性）
3. 使用 `--from-step 5` 从策略构建重新跑，跳过 step1-4（因子值不变）
...
```python
# Agent 伪代码（通过 run_pipeline 或 CLI）
state = run_pipeline(factor_id, from_step=1)  # 或 from_step=5 跳过 step1-4

if state.is_rejected():
    last = state.last_step()
    if last in ("step6", "step7"):
        # 策略调参：更新 config.yaml 或 CLI kwargs，从 step5 重跑
        state = run_pipeline(factor_id, from_step=5, top_k=100, decay=10, rebalance="1W")
    else:
        print("Factor rejected:", state.step_results[last].reason)
```
