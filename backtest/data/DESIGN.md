...
### `factors_daily`（因子宽表，研究主用）

- **物理库**：不在 `market.duckdb`，而是在 `data/duckdb/factors_pending.duckdb`（work）与 `data/duckdb/factor_library.duckdb`（library）。详见 [`backtest/factor/DESIGN.md`](../factor/DESIGN.md)。
- **Schema**：`(date DATE, symbol VARCHAR, <factor_id_1> DOUBLE, <factor_id_2> DOUBLE, ...)`
- **主键**：`(date, symbol)`
- 加新因子 = `ALTER TABLE ADD COLUMN <factor_id> DOUBLE`；每个 `factor_id` 在表里占一列，策略层直接读取宽表。
...
- **入库过滤**：保留合并口径全集 `report_type ∈ {1,2,3,4,5}`；母公司口径 `6 / 11 / 12` 不入库
- **主键**：`(symbol, end_date, f_ann_date, update_flag, report_type)`——同一 `(symbol, end_date, f_ann_date)` 可同时存在不同 `update_flag` / `report_type` 物理行，必须保留以支持 PIT 回放
...
- **版本语义**：
- `f_ann_date` 是唯一可靠的版本时间戳；`update_flag` 只作为同 `f_ann_date` 冗余行的稳定 tiebreaker
- `report_type` 暴露初始 / 基准 / 修正前等版本族；snapshot 不按 type 写分支，统一按最新可见 `f_ann_date` 取数
- 同一 `(symbol, end_date)` 可能有 1 行（无修正）或多行（不同 `f_ann_date` / `report_type` / `update_flag`）
...
```
pro.income / pro.balancesheet / pro.cashflow
  → DataFrame
  → 过滤 report_type ∈ {1,2,3,4,5}
  → rename ts_code→symbol
→ 各自独立 UPSERT 到对应物理表
   ON CONFLICT (symbol, end_date, f_ann_date, update_flag, report_type) DO UPDATE
```
...
- **读取时合并**：`get_fina_snapshot(D)` 内部对三张表分别做 PIT 快照，再按 `(symbol, end_date)` outer-join，非 key 列自动加 `inc_/bs_/cf_` 前缀
...
- 三张表各自 QUALIFY 后，按 `(symbol, end_date)` outer-join，非 key 列加 `inc_/bs_/cf_` 前缀
...
**对派生因子的影响**：离线计算财务因子时，先调用 `get_fina_snapshot(D)` 或批量版 `get_fina_snapshot_range()` 拿到 PIT wide DataFrame，再算因子。因子表只存最终因子值；`ann_date` / `f_ann_date` 等审计来源保留在三张财务原始表里。
...
>
> **当前状态**：data 层 Round 1 已落地（多 `report_type` fetch、5 列 PK、PIT snapshot）。单季度推导 / TTM / YoY 不在 data 层完成，由因子层 `backtest/factor/transforms.py` 助手函数承担（Round 2）。
...
| 功能 | 现状 | 正确做法 | 状态 |
|---|---|---|---|
| Fetch 多 `report_type` | `fetcher/fundamentals_fetcher.py` 已保留 1/2/3/4/5 | 保持合并口径全集 | 已落地 |
| Storage PK | `storage.py` 已使用 5 列 PK | `(symbol, end_date, f_ann_date, update_flag, report_type)` | 已落地 |
| Snapshot 取数 | 已按 `(f_ann_date DESC, update_flag DESC)` 取最新 | 保持现状 | 已落地 |
...
| 测试 | 已有 fundamentals / snapshot 范围测试 | + 单季度 + TTM + YoY 单测 | Round 2 |
...
- **Round 1（data 层）已落地**：现有 Barra 因子的输入语义已从 "仅 type=1 中最新" 变成 "type 1/2/3/4/5 全集中按 f_ann_date 最新"。多数情况一致，但**修正后版本族（type=5 或 update_flag=1）可能改写 type=1 之前的"原始已知值"**——这是石川 4-case 框架的正确行为；重跑历史回测时需预期轻微数值漂移。
...
基本面因子修正分两轮；Round 1 已完成，剩余工作进入 Round 2：
...
- (P0.4) backfill：代码改完后仍需在目标环境手动跑 `python -m backtest.data.backfill.fundamentals`（或重新 `cold_start`）刷新历史库
...
- (P0.7) 测试：单季度公式 + TTM 公式 + YoY 单测（Round 2）
...
FactorStorage.get_factor(factor_id, start, end)               # 因子表单因子时序（见 factor/storage.py）
FactorStorage.get_factor_panel(factor_ids, date)              # 因子宽表横截面（见 factor/storage.py）
get_dividends(symbols, start, end)                            # 分红事件
...
### 当前实现
- `storage.py` 已有 `trade_calendar` 物理表与 `insert_trade_calendar()` / `get_trade_dates_from_db()` / `get_rebalance_dates_from_db()`。
- `backfill_trade_calendar.py` 负责全量或增量写入，`cold_start.py` / `update_daily.py` 已在 Phase 0 调用。
- `trade_calendar.py` 走 DuckDB-first，空结果时 fallback 到 Tushare 并回写。
...
### `backtest/data/backfill_trade_calendar.py`
...
### `backtest/data/trade_calendar.py`

签名（保留旧 `get_trade_dates(start, end)` 名字，向后兼容）：
...
### `cold_start.py` 集成
主流水线**最开始**调用 `backfill_trade_calendar(start, end)`（必须先于 `market_daily` 回填，因为后者依赖交易日序列）。

### 增量更新
`update_daily.py` 在每日开始时调用 `backfill_trade_calendar(start=last_cal_date+1, end=today)`，确保新交易日入表。

### 完成状态
- [x] `trade_calendar` 表落地 + UPSERT 工作
- [x] `backfill_trade_calendar.py` CLI 可用
- [x] `cold_start.py` / `update_daily.py` 已接入
- [x] `get_trade_dates` DuckDB-first，Tushare 调用频率从每次 → 缺失时
- [x] `get_rebalance_dates(start, end, freq)` 已实现，测试覆盖 `1D / 5D / 1W / 2W / 1M / EOM`
- [x] `strategy/base.py` 已改为调用 `get_rebalance_dates`
