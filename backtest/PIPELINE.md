...
日期范围、horizons、ret_type 等统一从 `config.yaml` 读取，CLI 不再覆盖。
...
| `--decile` | off | `evaluation` 专用：**默认不跑**十段分层；加此 flag 才运行。十段单调性是 IC/策略回测之外的独立验证维度，强烈建议保留 |

> `--start` / `--end` / `--horizons` / `--ret-type` 已从 CLI 移除，统一由 `config.yaml` 的 `pipeline` 段指定。 |
...
> 十段分层和 IC/RankIC、策略回测（simple/detailed）是**三个独立的验证维度**。一个因子可以有高 IC 但分层不单调（如 `z(-ret) * z(turnover)` 这类四象限因子），也可以分层单调但 IC 一般（非线性关系）。三者互补，缺了分层图容易误判因子质量。
>
> **默认行为差异**：`scripts/run_factor_pipeline.py` **默认执行**十段分层（加 `--no-decile` 才跳过）；`backtest.factor.evaluation` **默认不执行**（加 `--decile` 才运行）。
...
```bash
# run_factor_pipeline.py 默认执行十段分层（--no-decile 才跳过）
python scripts/run_factor_pipeline.py f_xxx

# evaluation 默认不执行十段分层（--decile 才运行）
