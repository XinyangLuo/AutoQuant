...
- **评测模块**：从 simulation 落盘的 parquet 反推策略质量。收益/风险/胜率/交易/持仓指标 + 净值/回撤/月度热力图 + **自动多基准超额指标**（HS300/CSI500/CSI1000）。全项目指标计算单一真理源（`BacktestResult.summary()` 已退化为薄封装）。详见 [`backtest/evaluation/DESIGN.md`](evaluation/DESIGN.md)。
...
- **评测指标单一来源**：所有 Sharpe / 回撤 / 换手 / 胜率等指标只在 `backtest/evaluation/metrics.py` 实现。`BacktestResult.summary()` 委托到此，并**自动加载 HS300/CSI500/CSI1000 计算超额指标**（`excess_sharpe_*`、`excess_annual_return_*`、`excess_max_drawdown_*`、`excess_calmar_*`），杜绝公式漂移
